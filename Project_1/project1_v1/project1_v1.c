/* project 1, version 1 (Older: version 1.1, 1.2)
 * Name: Joshua Omori
 * Class: EE 160
 * Current objective: Make the polynomial work up to the 5th power (assuming no - or fraction powers).
...
double myfunc(double A, double B, double C, double D, double E, double F, double min) {
   return (A) + (B*min) + (C*min*min) + (D*min*min*min) + (E*min*min*min*min) + (F*min*min*min*min*min);
}
